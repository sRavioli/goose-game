 ...
 * static character array with a maximum length of `MAX_BUFFER_LEN`. This
 ...
 * the logger is active or not. A value of `TRUE` indicates that the logger is
 * active and can perform logging operations, while a value of `FALSE`
 ...
 * the logger has been started. It is set to `TRUE` when the logger is started
 ...
 *
 * The `pop_call()` function is used to remove the topmost function call from
 * the call stack. It checks if the call stack is empty and throws an error if
 * an attempt is made to pop a call when the call stack is already empty. The
